...
func (r *MySQLRepository)Save(pedido *domain.Pedido)(int64,error){
	...
	for _, detalle := range pedido.Detalles {
		_, err := r.conn.DB.Exec("INSERT INTO detalles_pedido (idPedido, idProducto, cantidad, precio_unitario, subtotal) VALUES (?, ?, ?, ?, ?)",
			pedidoID, detalle.IdProducto, detalle.Cantidad, detalle.PrecioUnitario, detalle.Subtotal)
		if err != nil {
			return 0,err
		}
	}
...
func (r *MySQLRepository)AgregarNuevoProducto(idPedido int,detalle *domain.DetallesPedido)error{
	_, err := r.conn.DB.Exec("INSERT INTO detalles_pedido (idPedido, idProducto, cantidad, precio_unitario, subtotal) VALUES (?, ?, ?, ?, ?)",
	idPedido, &detalle.IdProducto, &detalle.Cantidad, &detalle.PrecioUnitario, &detalle.Subtotal)
