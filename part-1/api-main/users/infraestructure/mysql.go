...
func (r *MySQLRepository) Save(p *domain.User) error {
	...
	_, err = r.conn.DB.Exec(query, &p.Name, &p.Last_name, &p.Email, &hashedPassword)
	return err
}
...
func (r *MySQLRepository) UpdateUser(id int, p *domain.User) error {
	...
	_, err = r.conn.DB.Exec(query, &p.Name, &p.Last_name, &p.Email, &hashedPassword, id)
	return err
}
...
func (r *MySQLRepository) GetUserByEmail(email string) (*domain.User, error) {
	query := "SELECT idUsuario, name, last_name, email, password FROM usuarios WHERE email = ? LIMIT 1"
	row := r.conn.DB.QueryRow(query, email)

	var user domain.User
	err := row.Scan(&user.ID, &user.Name, &user.Last_name, &user.Email, &user.Password)
	if err != nil {
...
func (r *MySQLRepository) Login(email string, password string) (*domain.User, string, error) {
	...
	query := "SELECT idUsuario, name, last_name, email, password FROM usuarios WHERE email = ?"
	row := r.conn.DB.QueryRow(query, email)

	var user domain.User
	if err := row.Scan(&user.ID, &user.Name, &user.Last_name, &user.Email, &user.Password); err != nil {
